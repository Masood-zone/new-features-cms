...
export const useUpdatePrepayment = () => {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: number; data: UpdatePrepaymentData }) =>
    ...
    onError: (error: any) => {
      console.error(error);
      const message =
        error?.response?.data?.message || "Failed to update prepayment.";
      toast.error(message);
    },
  });
};
...
export const useDeactivatePrepayment = () => {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      console.error(error);
      const message =
        error?.response?.data?.message || "Failed to deactivate prepayment.";
      toast.error(message);
    },
  });
};
...
export const useDeletePrepayment = () => {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      console.error(error);
      const message =
        error?.response?.data?.message || "Failed to delete prepayment.";
      toast.error(message);
    },
  });
};
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      if (data.deactivatedCount > 0) {
        toast.success(
          `${data.deactivatedCount} expired prepayments cleaned up successfully!`
        );
      } else {
    ...
    onError: (error: any) => {
      ...
      const message =
        ...
        "Failed to cleanup expired prepayments.";
      toast.error(message);
    },
  });
};
